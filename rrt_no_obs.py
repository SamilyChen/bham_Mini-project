#Stratigies:randomly generate 1 node in each loop
import sys
import random
import math
import pygame
...
NUMNODES = 5000			#Number of iteration/nodes you want
...
def main():
	...
	for i in range(NUMNODES):
		...
		#finding the node of rrt has nearest distance to goal position
		for p in nodes:
			if dist([p.x,p.y],[goal.x,goal.y])<dist([nn.x,nn.y],[goal.x,goal.y]):
				nn=p
		#compare the distance of random node to goal and nearest node of rrt
		if dist([nn.x,nn.y],[goal.x,goal.y])<dist([rand.x,rand.y],[goal.x,goal.y]):
	...
	print "times:%d",time.time()-time_start
